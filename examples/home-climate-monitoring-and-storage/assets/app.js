...
const temperatureLive = {
  canvas: null,
  chart: null,
  data: newChartData('orange', 'rgba(255,165,0,0.1)'),
  unit: '°C',
};
const humidityLive = {
  canvas: null,
  chart: null,
  data: newChartData('teal', 'rgba(0,128,128,0.08)'),
  unit: '%',
};
...
const dewPointLive = {
  canvas: null,
  chart: null,
  data: newChartData('blue', 'rgba(0,0,255,0.08)'),
  unit: '°C',
};
const heatIndexLive = {
  canvas: null,
  chart: null,
  data: newChartData('red', 'rgba(255,0,0,0.08)'),
  unit: '°C',
};
const absHumidityLive = {
  canvas: null,
  chart: null,
  data: newChartData('purple', 'rgba(128,0,128,0.06)'),
  unit: 'g/m³',
};
...
const tempPopoverText =
  'Shows temperature readings in °C. Data is average per 1h (1D view) or per 1 minute (1h view)';
const humidityPopoverText =
  'Shows relative humidity percentage. Data is average per 1h (1D view) or per 1 minute (1h view)';
const dewPointPopoverText =
  'Dew Point is the temperature at which air becomes saturated with moisture. It indicates the absolute humidity level.';
const heatIndexPopoverText =
  'Heat Index combines air temperature and relative humidity to determine the perceived temperature.';
const absHumidityPopoverText =
  'Absolute Humidity is the total amount of water vapor present in the air, expressed in grams per cubic meter (g/m³).';
...
function renderChartData(
  ...
  const noDataDiv = document.getElementById(
    (obj.canvas && obj.canvas.id) + '-nodata',
  );
  const liveCircle = document.getElementById('live-circle');
  const isLiveChart =
    obj.canvas && obj.canvas.id && obj.canvas.id.endsWith('-live-chart');
        ...
        tooltip: {
          ...
          callbacks: {
            ...
            label: function (context) {
              ...
              if (!context.chart.options._unit)
                context.chart.options._unit = obj.unit;
