...
This example runs entirely on the Arduino UNO Q??????????????????, utilizing its processing power for both the AI model and the web server.
...
- **`App.run()`**: Starts the Router Bridge runtime to keep the application and its services running.
