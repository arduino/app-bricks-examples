...
ui.on_message('classification_complete', handleClassificationResult);
ui.on_message('classification_error', handleClassificationError);
...
function renderSampleAudios() {
  ...
  if (!sampleAudios || sampleAudios.length === 0) {
    sampleAudiosGrid.innerHTML =
      '<div class="sample-audios-loading">No sample audios available</div>';
    return;
  }

  const html = sampleAudios
    .map(
      (audio, index) => `
...
function handleClassificationError(data) {
...
function runClassification() {
  ...
  if (currentAudioSource === 'upload') {
    if (currentAudio) {
      ...
      ui.send_message('run_classification', payload);
    } else {
