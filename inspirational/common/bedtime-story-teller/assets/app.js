...
function gatherDataAndGenerateStory() {
  ...
  const tone =
    storyTypeContainer.querySelector('.story-type-paragraph:nth-child(1) .chip.selected')?.textContent.trim() || 'any';
  const endingType =
  ...
  const narrativeStructure =
    storyTypeContainer.querySelector('.story-type-paragraph:nth-child(3) .chip.selected')?.textContent.trim() || 'any';
  const duration =
    storyTypeContainer.querySelector('.story-type-paragraph:nth-child(4) .chip.selected')?.textContent.trim() || 'any';
...
parameterContainers.forEach(container => {
  ...
  if (title === 'Story type') {
    ...
    container.querySelector('.other-textarea').addEventListener('input', () => unlockAndOpenNext(container), {
      once: true,
    });
  } else {
