...
The **Camera Adjustments** example captures a single image from an available camera, applies basic transformations (greyscaling and horizontal flipping), and saves it to local storage in JPEG format.
...
This example demonstrates how to capture images from a camera, apply transformations through a pipeline, and save them to local storage. The Python® script initializes a camera peripheral with a specific resolution (640x480), applies basic adjustments (greyscaling and horizontal flip) to the video stream, captures a single image, compresses it to JPEG format, and stores it as a file named `captured_image.jpg`. 

The application shows the fundamental workflow for working with camera hardware, including initialization, adjustment pipeline setup, image capture, compression, and file I/O operations.
...
The Python® script instantiates and starts the Camera object with a specified resolution and adjustment pipeline:

```python
    camera = Camera(resolution=(640, 480), adjustments=greyscaled() | flipped_h())
    camera.start()
```
...
The adjustments are added using the pipe operator (`|`) to create a transformation pipeline that is applied directly to the video stream from the camera. 

Multiple adjustments can be combined together to create complex transformation pipelines.

The full list of adjustements is in the package `arduino.app_utils.image.adjustments`.
...
```
Camera Hardware → Apply Adjustments → Capture → Compress (JPEG) → Save to File
```
...
The Python® component handles camera initialization with adjustments, image capture, compression, and file storage.
...
- **`from arduino.app_utils.image.adjustments import greyscaled, flipped_h`:** Imports adjustment functions to apply transformations to the camera stream. `greyscaled()` converts the image to grayscale, and `flipped_h()` flips the image horizontally.

- **`Camera(resolution=(640, 480), adjustments=greyscaled() | flipped_h())`:** Instantiates the Camera object with a specified resolution of 640x480 pixels and creates a transformation pipeline using the pipe operator (`|`). Multiple adjustments can be combined this way to create complex pipelines. All adjustments are applied directly to the video stream from the camera.
...
- **`camera.capture()`:** Captures a single raw image from the camera with all adjustments applied and returns it as a NumPy array.
