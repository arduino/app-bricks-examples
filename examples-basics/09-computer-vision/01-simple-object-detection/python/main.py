...
logger = Logger("ObjectDetectionApp")

object_detection = ObjectDetection()
pil_image = Image.open(f"{PATH_IMAGE}/{IMAGE_FILE}")
...
def loop():
    ...
    if results is None:
        ...
        logger.info(f"Detected {len(results.get('detection', []))} objects in the image in {diff:.2f} ms.")
        for idx, detection in enumerate(results.get("detection", [])):
            ...
            logger.info(f"Object {idx}: Label={label}, Confidence={confidence}")
        ...
        if img_with_boxes is not None:
            img_with_boxes.save("output_image.png", format="PNG")
