...
function initWebUI() {
  ...
  ui.on_message('code_detected', async (message) => {
  ...
  ui.on_message('frame_detected', async (message) => {
  ...
  ui.on_message('error', async (message) => {
    if (errorContainer) {
...
async function listScans() {
  try {
    ...
    if (!data.scans) {
      initialListErrorElement.textContent =
        'Received invalid data for initial scans.';
      initialListErrorElement.style.display = 'block';
    }
