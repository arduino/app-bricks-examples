...
The **Edge Dictation Assistant** example demonstrates HOW TO using the Arduino® VENTUNO™ Q. It uses a Large Language Model (LLM) to create a chatbot that helps you in your daily life. The board exploits its own hardware features to run the model locally, preserving the confidentiality of your data.

## Description

This App uses AI to dictate the audio captured with the microphone and the VENTUNO Q. It uses the `asr` Brick to connect to a local AI model (qwen3-4b) and the `web_ui` Brick to provide a chat-like interface.
