...
function initSocketIO() {
  ui.on_message('prompt', handlePrompt);
...
function unlockAndOpenNext(currentContainer) {
  ...
  if (
    nextContainer &&
    nextContainer.classList.contains('parameter-container')
  ) {
...
function gatherDataAndGenerateStory() {
  ...
  const age =
    document
      .querySelector('.parameter-container:nth-child(1) .chip.selected')
  ...
  const endingType =
    storyTypeContainer
      .querySelector('.story-type-paragraph:nth-child(2) .chip.selected')
      ?.textContent.trim() || 'any';
  const narrativeStructure =
    storyTypeContainer
      .querySelector('.story-type-paragraph:nth-child(3) .chip.selected')
      ?.textContent.trim() || 'any';
  const duration =
    storyTypeContainer
      .querySelector('.story-type-paragraph:nth-child(4) .chip.selected')
      ?.textContent.trim() || 'any';
...
function generateStory(data) {
  ...
  ui.send_message('generate_story', data);
}
...
parameterContainers.forEach((container) => {
  ...
  if (title === 'Story type') {
    ...
    container
      ...
      .addEventListener('input', () => unlockAndOpenNext(container), {
        once: true,
      });
  } else {
...
document
  .querySelector('.generate-story-button')
  .addEventListener('click', gatherDataAndGenerateStory);
